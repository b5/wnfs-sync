...
func deltaTreeString(d *fsdiff.Delta) string {
	p := treeprint.NewWithRoot(d.String())
	walkAddDelta(p, d)
	return p.String()
}

func walkAddDelta(p treeprint.Tree, d *fsdiff.Delta) {
	for _, ch := range d.Deltas {
		branch := p.AddNode(ch.String())
		walkAddDelta(branch, ch)
	}
}
